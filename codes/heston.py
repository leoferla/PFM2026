...
def simulate_heston (S0=66.0, delta0=0.08, mu=0.05, sigma1=0.30, kappa=2.0, alpha=0.06, sigma2=0.15,
    rho=-0.3, T=2.0, n_steps=2*252, n_paths=50, seed=None):
    ...
    for i in range(n_steps):
        ...
        for j in range (n_steps + 1):
            sigma2 = np.sqrt (max (0.0, d[j, i]))
            S[j, i + 1] = S[j, i] * np.exp (c1 + sigma2 * Z1[j])
...
def main ():
    ...
    times, S_paths, d_paths = simulate_heston (S0=S0, delta0=delta0, mu=mu, sigma1=sigma1,
        kappa=kappa, alpha=alpha, sigma2=sigma2, rho=rho, T=T, n_steps=n_steps, n_paths=n_paths,
        seed=None)
